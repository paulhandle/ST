...
- When an auth bug survives a plausible token-store fix, stop guessing and add diagnostics first. Backend access logs that only show `401 Unauthorized` are not enough; auth dependencies should log safe reason codes and response details should let the UI expose the precise failure without leaking raw tokens.
- Environment resets that delete `users` invalidate every browser token, but they cannot clear another device's localStorage. Treat backend `user_not_found` auth failures as a stale session: clear local auth state and force a fresh login instead of letting the user continue through onboarding with a token for a deleted user.
- Protected pages should validate real auth state at route entry, not only after individual page API calls fail. Use a shared app-shell gate that calls `/auth/me` before rendering protected content; keep API-level stale-session cleanup only as fallback.
- Before pushing follow-up work to an existing PR branch, check `gh pr view <number> --json state,mergedAt,headRefName`. If the PR is merged/closed, create a new branch from current `main` and open a fresh PR; do not keep pushing to the merged PR's head branch.
- First-run onboarding plan generation must be deterministic and fast. Do not let the default onboarding path implicitly call external LLMs just because `OPENAI_API_KEY` exists; require an explicit request opt-in such as `use_llm=true`, and keep the default path rule-based.
- New Alembic migrations must chain from the current migration head unless intentionally creating a branch. Always run `uv run alembic heads` and an empty-database `uv run alembic upgrade head` before pushing; a migration that points at an older revision can create multiple heads and break deploy migrations even if model-level tests pass.
- Maintenance scripts that create their own SQLAlchemy engine must normalize database URLs the same way as `app.core.config`. Fly commonly provides `postgres://...`; scripts must convert that to the app's supported `postgresql+psycopg://...` before calling `create_engine`, otherwise Postgres-specific logic is unreachable.
- Frontend auth tests must clear both mocked localStorage and auth cookies. `getToken()` treats the cookie as source of truth and can migrate `st_token` back into localStorage, so localStorage-only cleanup can leak a previous test's login state into no-passkey or logged-out scenarios.
- Always check PR state immediately before pushing to an existing PR branch, even if it was open earlier in the same session. If `gh pr view` reports `MERGED` or `CLOSED`, branch from current `origin/main` and cherry-pick the follow-up commit into a new PR.
