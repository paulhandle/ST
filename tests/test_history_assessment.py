...
os.environ["OPENAI_API_KEY"] = ""  # empty string: setdefault in load_local_env won't overwrite
...
class HistoryAssessmentWorkflowTestCase(unittest.TestCase):
    ...
    def test_import_history_and_assessment_workflow(self) -> None:
        ...
        history_resp = self.client.get(f"/athletes/{athlete_id}/history", headers=auth(self.token))
        self.assertEqual(200, history_resp.status_code)
        self.assertGreater(len(history_resp.json()), 0)

        assessment_resp = self.client.get(f"/athletes/{athlete_id}/assessment", headers=auth(self.token))
